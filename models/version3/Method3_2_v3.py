...
from .configuration_llama_v3 import Method3_2Config_v3
...
class Method3_2DecoderLayer_v3(Method1DecoderLayer_v3):
    """
    Method3_2的解码层：继承Method1的所有功能，仅修改MLP残差连接方式为可学习权重
    """
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        # 替换为Method3_2的MLP残差连接处理器
        self.modified_residual_mlp = Method3_2ModifiedResidualMLP(layer_idx, config.num_hidden_layers)
    
    def get_layer_weights(self) -> torch.Tensor:
        """
        获取当前层的MLP权重分布
        """
        return self.modified_residual_mlp.get_layer_weights()


class Method3_2LlamaModel_v3(Method1LlamaModel_v3):
    """
    Method3_2的模型：继承Method1的所有功能，仅使用不同的解码层
    """
    config_class = Method3_2Config_v3

    def __init__(self, config: Method3_2Config_v3):
        # 先调用LlamaModel的初始化（跳过Method1LlamaModel_v3的初始化以避免重复）
        LlamaModel.__init__(self, config)
        # 替换所有的decoder layer为Method3_2的实现
        self.layers = nn.ModuleList(
            [Method3_2DecoderLayer_v3(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Method3_2LlamaForCausalLM_v3(LlamaForCausalLM):
    """
    Method3_2的因果语言模型
    """
    config_class = Method3_2Config_v3
    ...
    def __init__(self, config: Method3_2Config_v3):
        ...
        self.model = Method3_2LlamaModel_v3(config)
