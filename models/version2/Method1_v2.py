...
from .configuration_llama_v2 import LlamaConfig, Method1Config_v2

class Method1DecoderLayer_v2(LlamaDecoderLayer):
...
class Method1LlamaModel_v2(LlamaModel):
    config_class = Method1Config_v2

    def __init__(self, config: Method1Config_v2):
        super().__init__(config)
        # 替换所有的decoder layer为新的实现
        self.layers = nn.ModuleList(
            [Method1DecoderLayer_v2(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Method1LlamaForCausalLM_v2(LlamaForCausalLM):
    config_class = Method1Config_v2

    def __init__(self, config: Method1Config_v2):
        super(LlamaForCausalLM, self).__init__(config)
        self.model = Method1LlamaModel_v2(config)
