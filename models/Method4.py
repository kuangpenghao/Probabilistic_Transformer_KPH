...
from .configuration_llama import LlamaConfig, Method4LlamaConfig

class Method4DecoderLayer(LlamaDecoderLayer):
    """
        ...
        **kwargs,
        ...
        if self.layer_idx == 0:
            ...
            if previous_mlp_outputs is not None and len(previous_mlp_outputs) > 0:
                ...
                residual_avg = residual_sum / len(previous_mlp_outputs)  # 计算平均值
                hidden_states = residual_avg + mlp_output
            else:
                # 如果没有提供之前的输出，回退到原始行为
...
class Method4LlamaModel(LlamaModel):
    """
    实现新MLP残差连接方式的LlamaModel
    """
    config_class = Method4LlamaConfig

    def __init__(self, config: Method4LlamaConfig):
        super().__init__(config)
        # 替换所有的decoder layer为新的实现
        self.layers = nn.ModuleList(
            [Method4DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Method4LlamaForCausalLM(LlamaForCausalLM):
    """
    实现新MLP残差连接方式的因果语言模型
    """
    config_class = Method4LlamaConfig
    ...
    def __init__(self, config: Method4LlamaConfig):
        ...
        self.model = Method4LlamaModel(config)
