...
from .configuration_llama_v4 import Method1DConfig_v4
...
class v4m1D_ModifiedScailingComputation(nn.Module):
    """
    封装修改后的注意力缩放计算逻辑
    将原始的sqrt(d_k)广播成向量，与之前所有层QK^T组成的向量进行点乘
    每个QK^T矩阵都有对应的可学习权重列向量，但这些列向量不是直接学习的，
    而是由输入嵌入X_i通过MLP动态生成：A_i = GELU(RMSNorm(X_i)W_1)W_2 + bias
    
    第i层Transformer拥有i+1个权重列向量（对应前面i+1层的QK^T矩阵）
    这些列向量组成矩阵A_i，规模为T*(i+1)，由输入嵌入动态生成
    """
    def __init__(self, hidden_size: int, head_dim: int, num_layers: int):
        ...
        for layer_idx in range(num_layers):
            # 该层需要生成 (layer_idx + 1) 个列向量
            output_dim = layer_idx + 1
            
            # RMSNorm
            layer_norm = nn.RMSNorm(hidden_size)
            
            # MLP: hidden_size -> 4*hidden_size -> (layer_idx + 1)
            mlp = nn.Sequential(
    ...
    def compute_modified_scaling(self, qk_matrices: List[torch.Tensor], layer_idx: int, 
        ...
        # 对所有QK^T矩阵进行加权求和，每个矩阵乘以其对应的动态生成的权重列向量
        ...
        for i, qk_matrix in enumerate(qk_matrices):
            ...
            # 显式列向量广播：[batch_size, seq_len] -> [batch_size, seq_len, 1]
            # 然后与qk_matrix相乘: [batch_size, num_heads, seq_len, seq_len]
            exp_weight_expanded = exp_weight_column.unsqueeze(-1)  # [batch_size, seq_len, 1]
            
            # qk_matrix: [batch_size, num_heads, seq_len, seq_len]
            # exp_weight_expanded: [batch_size, seq_len, 1] -> 广播到 [batch_size, 1, seq_len, 1]
            exp_weight_broadcast = exp_weight_expanded.unsqueeze(1)  # [batch_size, 1, seq_len, 1]
...
class Method1DLlamaAttention_v4(Method1LlamaAttention_v4):
    """
    自定义Attention类，使用动态生成权重列向量的ModifiedScailingComputation
    """
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        # 替换为新的缩放计算模块
        self.modified_scaling = v4m1D_ModifiedScailingComputation(
...
class Method1DDecoderLayer_v4(Method1DecoderLayer_v4):
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        # 使用自定义的Attention类
        self.self_attn = Method1DLlamaAttention_v4(config=config, layer_idx=layer_idx)


class Method1DLlamaModel_v4(Method1LlamaModel_v4):
    config_class = Method1DConfig_v4

    def __init__(self, config: Method1DConfig_v4):
        super().__init__(config)
        # 替换所有的decoder layer为新的实现
        self.layers = nn.ModuleList(
            [Method1DDecoderLayer_v4(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Method1DLlamaForCausalLM_v4(Method1LlamaForCausalLM_v4):
    config_class = Method1DConfig_v4

    def __init__(self, config: Method1DConfig_v4):
        super().__init__(config)
        self.model = Method1DLlamaModel_v4(config)
