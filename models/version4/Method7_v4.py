...
from .configuration_llama_v4 import Method7Config_v4
...
class v4m7_ModifiedScailingComputation(nn.Module):
    """
    封装修改后的注意力缩放计算逻辑
    学习一个向量，向量的每一项为a_i，其中a_i是可学习参数
...
class Method7LlamaAttention_v4(Method1LlamaAttention_v4):
    """
    继承Method1的Attention类，只修改ModifiedScalingComputation
    """
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        # 替换为Method7的缩放计算，每层都有自己独立的参数向量
        self.modified_scaling = v4m7_ModifiedScailingComputation(self.head_dim, layer_idx)


class Method7DecoderLayer_v4(Method1DecoderLayer_v4):
    """
    继承Method1的DecoderLayer类，只修改Attention模块
    """
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        # 替换为Method7的Attention类
        self.self_attn = Method7LlamaAttention_v4(config=config, layer_idx=layer_idx)


class Method7LlamaModel_v4(Method1LlamaModel_v4):
    """
    继承Method1的Model类，只修改config和decoder layers
    """
    config_class = Method7Config_v4

    def __init__(self, config: Method7Config_v4):
        super().__init__(config)
        # 替换为Method7的decoder layers
        self.layers = nn.ModuleList(
            [Method7DecoderLayer_v4(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )


class Method7LlamaForCausalLM_v4(Method1LlamaForCausalLM_v4):
    """
    ...
    def __init__(self, config: Method7Config_v4):
        super().__init__(config)
        self.model = Method7LlamaModel_v4(config)
    ...
    def save_learned_parameters(self, output_dir: str):
        ...
        # 保存到文件
        weights_file = os.path.join(output_dir, "method7_v4_learned_parameters.json")
        ...
        with open(stats_file, 'w') as f:
            f.write("Method7_v4 Learned Parameters\n")
            f.write("="*40 + "\n")
            f.write(f"Total layers: {len(weights_data)}\n")
            f.write(f"Parameters: Vector a_i for scaling 1/a_i\n\n")
            ...
            for layer_idx, layer_weights in enumerate(all_weights):
                if len(layer_weights) > 0:
                    ...
                    f.write(f"  Scaling formula: weighted sum with 1/a_i\n")
                    f.write("-" * 30 + "\n")
