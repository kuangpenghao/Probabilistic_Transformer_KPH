...
from .configuration_llama_v4 import Method3Config_v4
...
class v4m3_ModifiedScailingComputation(nn.Module):
    """
    封装修改后的注意力缩放计算逻辑
...
class Method3LlamaAttention_v4(Method1LlamaAttention_v4):
    """
    ...
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        # 替换为Method3的缩放计算
        self.modified_scaling = v4m3_ModifiedScailingComputation(self.head_dim)


class Method3DecoderLayer_v4(Method1DecoderLayer_v4):
    """
    继承Method1的DecoderLayer类，只修改Attention模块
    """
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        # 替换为Method3的Attention类
        self.self_attn = Method3LlamaAttention_v4(config=config, layer_idx=layer_idx)


class Method3LlamaModel_v4(Method1LlamaModel_v4):
    """
    继承Method1的Model类，只修改config和decoder layers
    """
    config_class = Method3Config_v4

    def __init__(self, config: Method3Config_v4):
        super().__init__(config)
        # 替换为Method3的decoder layers
        self.layers = nn.ModuleList(
            [Method3DecoderLayer_v4(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )


class Method3LlamaForCausalLM_v4(Method1LlamaForCausalLM_v4):
    """
    ...
    def __init__(self, config: Method3Config_v4):
        super().__init__(config)
        self.model = Method3LlamaModel_v4(config)
