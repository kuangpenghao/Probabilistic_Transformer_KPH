...
from .configuration_llama_v4 import Method1CConfig_v4
...
class v4m1C_ModifiedScailingComputation(nn.Module):
    """
    封装修改后的注意力缩放计算逻辑
    将原始的sqrt(d_k)广播成向量，与之前所有层QK^T组成的向量进行点乘
    每个QK^T矩阵都有对应的可学习权重列向量，广播成矩阵后进行逐元素相乘
    
    第i层Transformer拥有i+1个可学习权重列向量（对应前面i+1层的QK^T矩阵）
    使用ModuleList[ParameterList]存储每层的权重列向量列表
    """
    def __init__(self, head_dim: int, num_layers: int):
        super().__init__()
        self.head_dim = head_dim
        self.num_layers = num_layers
        # 为每一层创建权重列向量ParameterList，第i层有i+1个权重列向量
    ...
    def _initialize_layer_weights(self, layer_idx: int, qk_shapes: List[torch.Size], device: torch.device, dtype: torch.dtype):
        ...
        # 为该层的每个QK^T矩阵创建对应的权重列向量
        for qk_shape in qk_shapes:
            # 创建权重列向量，长度为QK^T矩阵的行数，初始化为0（经过exp后为1）
            weight_vector = nn.Parameter(torch.zeros(qk_shape[2], device=device, dtype=dtype))
    ...
    def compute_modified_scaling(self, qk_matrices: List[torch.Tensor], layer_idx: int) -> torch.Tensor:
        ...
        # 对所有QK^T矩阵进行加权求和，每个矩阵乘以其对应的可学习权重列向量
        ...
        for i, qk_matrix in enumerate(qk_matrices):
            ...
            exp_weight_column = exp_weight_vector.unsqueeze(1)  # [seq_len] -> [seq_len, 1]
            weighted_qk += scaling_vector[i] * (qk_matrix * exp_weight_column)
        
        return weighted_qk


class Method1CLlamaAttention_v4(Method1LlamaAttention_v4):
    """
    自定义Attention类，使用带可学习权重列向量的ModifiedScailingComputation
    """
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        # 替换为新的缩放计算模块
        self.modified_scaling = v4m1C_ModifiedScailingComputation(self.head_dim, config.num_hidden_layers)


class Method1CDecoderLayer_v4(Method1DecoderLayer_v4):
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        # 使用自定义的Attention类
        self.self_attn = Method1CLlamaAttention_v4(config=config, layer_idx=layer_idx)


class Method1CLlamaModel_v4(Method1LlamaModel_v4):
    config_class = Method1CConfig_v4

    def __init__(self, config: Method1CConfig_v4):
        super().__init__(config)
        # 替换所有的decoder layer为新的实现
        self.layers = nn.ModuleList(
            [Method1CDecoderLayer_v4(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Method1CLlamaForCausalLM_v4(Method1LlamaForCausalLM_v4):
    config_class = Method1CConfig_v4

    def __init__(self, config: Method1CConfig_v4):
        super().__init__(config)
        self.model = Method1CLlamaModel_v4(config)
