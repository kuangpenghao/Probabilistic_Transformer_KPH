...
class Method7DecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        # 新增参数：存储之前层的注意力输出和MLP输出
        previous_attn_outputs: Optional[List[torch.Tensor]] = None,
        previous_mlp_outputs: Optional[List[torch.Tensor]] = None,
        **kwargs,
    ) -> Tuple[torch.FloatTensor, Optional[Tuple[torch.FloatTensor, torch.FloatTensor]], Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        # 添加当前层的注意力输出和MLP输出到返回值中
        outputs += (current_attn_output, current_mlp_output)
...
class Method7LlamaModel(LlamaModel):
    ...
    def __init__(self, config: Method7LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [Method7DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Method7LlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config: Method7LlamaConfig):
        ...
        self.model = Method7LlamaModel(config)
